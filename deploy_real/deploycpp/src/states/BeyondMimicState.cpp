...
std::vector<T> requiredVector(const YAML::Node& node, const std::string& key) {
  if (!node[key]) {
    throw std::runtime_error("Missing BeyondMimic key: " + key);
  }
...
std::vector<float> flattenMat3(const Mat3& m) {
...
BeyondMimicState::BeyondMimicState(const Config& config)
    : runner_(config.beyond_mimic_model_path) {
  loadYaml(config.beyond_mimic_yaml_path);
}

FSMStateName BeyondMimicState::name() const {
  return FSMStateName::BeyondMimic;
}

void BeyondMimicState::enter(const StateAndCmd& state, PolicyOutput& output) {
  counter_step_ = 0;
...
void BeyondMimicState::run(const StateAndCmd& state, PolicyOutput& output) {
  ...
  if (counter_step_ < 2) {
    ...
    ++counter_step_;
    writeHoldOutput(state, output);
    std::cout << "beyond_mimic_anchor_frame=" << counter_step_ << "\n";
    return;
  }
  ...
  ++counter_step_;
  if (counter_step_ == 3) {
    std::cout << "beyond_mimic_policy_frame=3\n";
  }
}

void BeyondMimicState::exit(const StateAndCmd&, PolicyOutput&) {
  counter_step_ = 0;
}

FSMStateName BeyondMimicState::checkChange(const StateAndCmd& state) const {
  if (state.remote.F1) {
...
BeyondMimicAlignmentDump BeyondMimicState::dumpAlignmentFixture() {
  const std::array<float, 29> q = deterministicQ();
  const std::array<float, 29> dq = deterministicDq();
  const std::array<float, 3> ang_vel = deterministicAngVel();

  std::vector<float> zero_obs(static_cast<std::size_t>(num_obs_), 0.0f);
  PolicyRunResult warmup = runner_.run(zero_obs, 0.0f);

  std::vector<float> qj_mj2lab;
  const Quat robot_quat = torsoAdjustedQuat(q, deterministicBaseQuat(), qj_mj2lab);
  const Quat ref_anchor_ori_w{
      warmup.body_quat_w[7 * 4 + 0],
      warmup.body_quat_w[7 * 4 + 1],
      warmup.body_quat_w[7 * 4 + 2],
      warmup.body_quat_w[7 * 4 + 3],
  };
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    init_to_world = multiply(world_to_anchor, transpose(init_to_anchor));
  }

  const Mat3 motion_anchor_ori_b =
      multiply(multiply(transpose(matrixFromQuat(robot_quat)), init_to_world), matrixFromQuat(ref_anchor_ori_w));

  current_result_ = warmup;
  const std::vector<float> mimic_obs = buildMimicObs(qj_mj2lab, dq, ang_vel, motion_anchor_ori_b);

  const PolicyRunResult result = runner_.run(mimic_obs, 2.0f);
  std::vector<float> target_dof_pos_mj(29, 0.0f);
  for (std::size_t i = 0; i < mj2lab_.size(); ++i) {
    const float target_lab = result.actions[i] * action_scale_lab_[i] + default_angles_lab_[i];
    target_dof_pos_mj[static_cast<std::size_t>(mj2lab_[i])] = target_lab;
  }

  BeyondMimicAlignmentDump dump;
  dump.qj_mj2lab = qj_mj2lab;
  dump.robot_quat = {robot_quat[0], robot_quat[1], robot_quat[2], robot_quat[3]};
  dump.motion_anchor_ori_b = flattenMat3(motion_anchor_ori_b);
  dump.mimic_obs = mimic_obs;
  dump.target_dof_pos_mj = target_dof_pos_mj;
  return dump;
}

int BeyondMimicState::counterStep() const {
  return counter_step_;
}

std::array<float, 29> BeyondMimicState::deterministicQ() {
  std::array<float, 29> q{};
  for (std::size_t i = 0; i < q.size(); ++i) {
    q[i] = -0.20f + 0.015f * static_cast<float>(i);
  }
  return q;
}

std::array<float, 29> BeyondMimicState::deterministicDq() {
  std::array<float, 29> dq{};
  for (std::size_t i = 0; i < dq.size(); ++i) {
    dq[i] = 0.001f * (static_cast<float>(i) - 14.0f);
  }
  return dq;
}

Quat BeyondMimicState::deterministicBaseQuat() {
  return {0.997851f, 0.0137651f, -0.000958114f, -0.0640478f};
}

std::array<float, 3> BeyondMimicState::deterministicAngVel() {
  return {-0.00213053f, -0.00426106f, -0.00213053f};
}

void BeyondMimicState::loadYaml(const std::string& yaml_path) {
  ...
  if (!root["num_obs"]) {
    throw std::runtime_error("Missing BeyondMimic key: num_obs");
  }
  ...
  if (mj2lab_.size() != 29 || kps_lab_.size() != 29 || kds_lab_.size() != 29 ||
    ...
    throw std::runtime_error("BeyondMimic config arrays must all have size 29");
  }
}

Quat BeyondMimicState::torsoAdjustedQuat(
...
std::vector<float> BeyondMimicState::buildMimicObs(
  ...
  if (mimic_obs.size() != 154) {
    throw std::runtime_error("Expected mimic_obs size 154, got " + std::to_string(mimic_obs.size()));
  }
  return mimic_obs;
}

Mat3 BeyondMimicState::computeMotionAnchor(const Quat& robot_quat) const {
  const Quat ref_anchor_ori_w{
      current_result_.body_quat_w[7 * 4 + 0],
      current_result_.body_quat_w[7 * 4 + 1],
      current_result_.body_quat_w[7 * 4 + 2],
...
void BeyondMimicState::writeHoldOutput(const StateAndCmd& state, PolicyOutput& output) const {
...
void BeyondMimicState::writePolicyOutput(const PolicyRunResult& result, PolicyOutput& output) const {
  for (std::size_t i = 0; i < mj2lab_.size(); ++i) {
    const std::size_t motor_idx = static_cast<std::size_t>(mj2lab_[i]);
