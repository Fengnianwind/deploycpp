...
class BeyondMimic2(FSMState):
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        super().__init__()
        self.state_cmd = state_cmd
        self.policy_output = policy_output
        self.name = FSMStateName.SKILL_BEYOND_MIMIC2
        self.name_str = "beyond_mimic2"
        self.motion_phase = 0
        self.counter_step = 0
        self.ref_motion_phase = 0
        
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "BeyondMimic2.yaml")
        with open(config_path, "r") as f:
            ...
            # 添加缺失的属性
            self.history_length = 1  # BeyondMimic可能不需要历史数据，设为1
    ...
    def run(self):
        ...
        # update motion phase and show progress bar
        self.counter_step += 1
    ...
    def checkChange(self):
        if(self.state_cmd.skill_cmd == FSMCommand.LOCO):
            ...
            self.state_cmd.skill_cmd = FSMCommand.INVALID
            return FSMStateName.SKILL_BEYOND_MIMIC2
